...
static void hiscore_load (running_machine &machine)
{
	...
	if (filerr == FILERR_NONE)
	{
        ...
        mame_printf_verbose("Loading Hi:");
		...
		while (mem_range)
		{
			...
			if (data)
			{
				...
				mame_printf_verbose("%X ",(unsigned int)data);
...
static void hiscore_file_cache (running_machine &machine)
{
	...
	file_error filerr;
  	...
  	filerr = f.open(machine.basename(), "-cache.hi");

	if (filerr == FILERR_NONE)
	{
		memory_range *mem_range = state.mem_range;

    	mame_printf_verbose("Saving Cache Hi: ");
...
/* CAMILO -- Reads hiscore and output to stdout Every Frame */
void hiscore_print(running_machine &machine)
{
        if(counter <= DELAY) return;
        
        memory_range *mem_range = state.mem_range;

        mame_printf_verbose("Game Hi: ");
	...
	if (state.mem_range)
	{
		if (!state.hiscores_have_been_loaded)
		{
			if (safe_to_load(machine))
			{
		    	...
		    	// CAMILO -- Go for verbose reading of highscore every frame
	            hiscore_print(machine);
	            hiscore_file_cache(machine);
	            
	            // This is for paused printing
                if(counter <= DELAY){
                    counter++;
                } else {
                    counter = 0;
